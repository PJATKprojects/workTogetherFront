...
import { useNotificationUnread } from "@/hooks/use-notifications";
...
export function AuthNavActions({ locale, nav }: Props) {
  ...
  const unreadNotifications = useNotificationUnread(isAuthenticated).data ?? 0;
      ...
      <>
        {user?.isAdmin ? (
          <Link
            ...
            className="focus-ring hidden whitespace-nowrap rounded-full border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm font-semibold text-amber-700 transition-colors hover:bg-amber-500/20 dark:text-amber-300 lg:inline-flex"
          >
            {localText(locale, "Admin", "Адмін", "Administracja")}
          </Link>
        ) : null}
        <Link
          href={withLocale(locale, "/notifications")}
          aria-label={nav.notifications}
          className="focus-ring relative hidden rounded-full px-2.5 py-2 text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground lg:inline-flex"
        >
          {nav.notifications}
          {unreadNotifications > 0 ? (
            <span className="absolute -right-0.5 -top-0.5 flex h-4.5 min-w-4.5 items-center justify-center rounded-full bg-destructive px-1 text-[10px] font-bold text-primary-foreground">
