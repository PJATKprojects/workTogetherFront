...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!isLocale(locale)) return { title: "Safety — WorkTogether" };
  const t = getMessages(locale).safety;
  ...
  return (
    <div className="flex min-h-full flex-col text-foreground">
      ...
      <main id="main-content" tabIndex={-1} className="flex-1">
        <LegalArticle doc={t.safety} shared={t.legal} identity={getLegalIdentity()} />
        <IllegalContentNoticeForm locale={locale} labels={t.illegalContentForm} />
      </main>
